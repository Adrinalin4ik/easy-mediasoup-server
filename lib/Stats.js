	...
	async getWorkerStat(pid, worker) {
		...
		if (config.telegram.enabled && stats.cpu > config.telegram.cpuNotificationTheshold) {
			let rooms = Array.from(worker._rooms.values()).map(r => {
				...
				if (room) {
					const roomId = room._roomId.replace('trainingspace_', '');
					console.log(config.telegram.roomLink)
					console.log(config.telegram.roomLink.replace('{{roomId}}', roomId))
					return `[${roomId}](${config.telegram.roomLink.replace('{{roomId}}', roomId)})`
				}
			})
