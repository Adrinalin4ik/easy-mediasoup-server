...
class Room extends EventEmitter
{
	...
	_handleProtooPeer(nsp)
	{
		logger.debug(`_handleProtooPeer() [peer:"%s"]`, nsp);

		nsp.on('mediasoup-request', (request) => {
			console.log('mediasoup-request', request)
			this._handleMediasoupClientRequest(
						nsp, request);
		})

		nsp.on('mediasoup-notification', (request) => {
			logger.debug('mediasoup-notification', request)
			this._handleMediasoupClientNotification(
						nsp, request);
		})

		nsp.on('request', (request, accept, reject) =>
		{
			logger.debug(
				'protoo "request" event [method:%s, peer:"%s"]',
				request.method, nsp.id);
	...
	_handleMediasoupClientRequest(nsp, request)
	{
		logger.debug(
