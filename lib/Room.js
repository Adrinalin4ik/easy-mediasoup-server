...
class Room extends EventEmitter {
	constructor(roomId, mediaServer) {
		...
		this.send_active_speakers_time_interval = config.send_active_speakers_time_interval;
		this.active_speaker_db_level = config.active_speaker_db_level;
		this.send_active_speakers_timer = null;
		// Room ID.
		this._roomId = roomId;
	...
	close() {
		logger.debug('close()');
		clearInterval(this.send_active_speakers_timer)
	...
	_activeSpeakersWorker() {
		this.send_active_speakers_timer = setInterval(() => {
			this._protooRoom.spread('active-speakers', this.active_speakers);
			this.active_speakers = {};
		}, this.send_active_speakers_time_interval)
		
	}

	_handleMediaRoom() {
		logger.debug('_handleMediaRoom()');
		if (config.active_enable_check_active_speakers){
			this._mediaRoom.on('audiolevels', (audioLevelInfos) => {
				for(let speaker of audioLevelInfos){
					if (speaker.audioLevel > -35) {
						this.active_speakers[speaker.producer.peer.name] = speaker.audioLevel
					}
				}
			})
			this._activeSpeakersWorker();
		}
		...
		protooPeer.on('request', (request, accept, reject) => {
			...
			switch (request.method) {
				...
				case 'message': {
					...
					this._protooRoom.spread('message', request.data);
